...
async function createModulePackages({ from, to }) {
  ...
  await Promise.all(
    directoryPackages.map(async (dirName) => {
       ...
       */
      const mainFileName = fs.existsSync(path.join(to, dirName, 'index.js'))
        ? 'index.js'
        : `${dirName}.js`;

      const mainDeclarationExists = fs.existsSync(path.join(to, dirName, 'index.d.ts'));
      ...
      if (mainFileName !== 'index.js') {
        ...
        if (!fs.existsSync(path.join(to, dirName, mainFileName))) {
      ...
      const [ typingsExist ] = await Promise.all([
        ...
        fs.existsSync(typingsPath),
        /** Write the package.json */
        new Promise((resolve, reject) => (
          fs.writeFile(packageJsonPath, JSON.stringify(modulePackage, null, 2), (error) => error ? reject(error) : resolve())
        ))
      ]);
